...
q_list = G.q_list;                                                          % phase 1
%if variant == 1
%    q_list(:, 3) = 0.5;
%end
q_keep_indx = find(A(q_list(:, 1)) == A(q_list(:, 2)));
q_list = q_list(q_keep_indx, :);                                            % end of phase 1
% Select edges at random based on q_list
selected_edges_q_list_indx = find(q_list(:, 3) > rand(size(q_list,1), 1));  % phase 2
...
SelEdgeMat = sparse([selected_edges(:,1)'; selected_edges(:,2)'],...
                    ...
                    1, length(G.names), length(G.names));          % This represents the linked variables
                                                                   % after selection
[var2comp, cc_sizes] = scomponents(SelEdgeMat);                    % cc are clusters, phase 2
num_cc = length(cc_sizes);


% Select a connected component (the book calls this Y)
selected_cc = ceil(rand() * num_cc);                               % component label, selecting component from 2nd phase
selected_vars = find(var2comp == selected_cc);                     % generated cluster of variables
%disp(selected_vars)
...
elseif variant == 2                                                        % phase 3
    ...
    %for k = 1:length(selected_edges)
    %    factor_indice = E2F{selected_edges(k, 1), selected_edges(k, 2)};
    %    Factor = F(factor_indice);
    %    t = transpose(1:d);
    %    ass = repmat(t, 1, length(Factor.var));
    %    num = sum(GetValueOfAssignment(Factor, ass));
    %    den = sum(Factor.val);
    %    [~, ind] = ismember(selected_edges(k, 1:2), G.q_list(:, 1:2), 'rows');
    %    G.q_list(ind, 3) = num / den;
    %end
...
if variant == 1
    ...
    val = LogR(old_value);
    valp = LogR(new_value);
    Q_Ratio = QY_Ratio * exp(val) / exp(valp);
end
p_acceptance = min([1, Q_Ratio * PIratio]);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
