...
if bSwendsenWang
  ...
  for i = 1:size([u, v], 1)
      ...
      if u(i) > v(i)
          edge_factor = F(E2F{u(i), v(i)}(1));                             % We take the first incoming factor
          disp(edge_factor)
          q_ij = 0.0;
          ...
          elseif strcmp(TransName, 'MHSwendsenWang2')
              ...
              %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
              t = transpose(1:edge_factor.card(1));
              ass = repmat(t, 1, length(edge_factor.var));
              num = sum(GetValueOfAssignment(edge_factor, ass));
