...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
for i = 1:numPeople
    parent1_copy1 = pedigree.parents(i, 1);
    parent1_copy2 = parent1_copy1 + numPeople;
    parent2_copy1 = pedigree.parents(i, 2);
    parent2_copy2 = parent2_copy1 + numPeople;
    if parent1_copy1 == 0 || parent2_copy1 == 0
        factorList(i).var = i;
        factorList(i).card = numAlleles;
        factorList(i).val = transpose(alleleFreqs);
        factorList(i + numPeople).var = i + numPeople;
        factorList(i + numPeople).card = numAlleles;
        factorList(i + numPeople).val = transpose(alleleFreqs);
    else
        factorList(i).var = [i, parent1_copy1, parent1_copy2];
        factorList(i).card = [numAlleles, numAlleles,numAlleles];
        factorList(i).val = zeros(1, prod(factorList(i).card));
        factorList(i + numPeople).var = [i, parent2_copy1, parent2_copy2];
        factorList(i + numPeople).card = [numAlleles, numAlleles,numAlleles];
        factorList(i + numPeople).val = zeros(1, prod(factorList(i + numPeople).card));
        for j = 1:length(factorList(i).val)
            ind1 = IndexToAssignment(j, factorList(i).card);
            if ind1(1) == ind1(2) && ind1(1) == ind1(3)
                factorList(i).val(j) = 1;
            elseif ind1(1) == ind1(2) && ind1(1) ~= ind1(3)
                factorList(i).val(j) = 0.5;
            elseif ind1(1) ~= ind1(2) && ind1(1) == ind1(3)
                factorList(i).val(j) = 0.5;
            end
            ind2 = IndexToAssignment(j, factorList(i + numPeople).card);
            if ind2(1) == ind2(2) && ind2(1) == ind2(3)
                factorList(i + numPeople).val(j) = 1;
            elseif ind2(1) == ind2(2) && ind2(1) ~= ind2(3)
                factorList(i + numPeople).val(j) = 0.5;
            elseif ind2(1) ~= ind2(2) && ind2(1) == ind2(3)
                factorList(i + numPeople).val(j) = 0.5;
            end
        end
...
for k = 2*numPeople+1:3*numPeople
    factorList(k) = phenotypeGivenCopiesFactor(alphaList, numAlleles, k-2*numPeople, k-numPeople, k);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  