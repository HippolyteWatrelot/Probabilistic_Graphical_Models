...
phenotypeFactor.var = cat(2, phenotypeVar, transpose(geneCopyVarOneList), transpose(geneCopyVarTwoList));
phenotypeFactor.card = cat(2, 2, length(alleleWeights{1}) * ones(1, 2 * length(geneCopyVarOneList)));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
...
for i = 1:prod(phenotypeFactor.card)
    ind = IndexToAssignment(i, phenotypeFactor.card);
    weight = 0;
    for j = 2:1+length(alleleWeights{1})
        weight = weight + alleleWeights{j - 1}(ind(j));
    end
    for j = 2+length(alleleWeights{1}):1+length(alleleWeights{1})+length(alleleWeights{2})
        weight = weight + alleleWeights{j - 1 - length(alleleWeights{1})}(ind(j));
    end
    if ind(1) == 1
